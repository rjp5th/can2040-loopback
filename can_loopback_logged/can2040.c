...
{
    TRICE( ID(12270),"PIO_LL: Sync SM Setup\n");
...
{
    TRICE( ID(11651),"PIO_LL: RX SM Setup\n");
    pio_hw_t *pio_hw = cd->pio_hw;
...
{
    TRICE( ID(12763),"PIO_LL: Match SM Setup\n");
    pio_hw_t *pio_hw = cd->pio_hw;
...
{
    TRICE( ID(12900),"PIO_LL: TX SM Setup\n");
    pio_hw_t *pio_hw = cd->pio_hw;
...
{
    TRICE( ID(15044),"PIO_LL: sync normal start signal\n");
    pio_hw_t *pio_hw = cd->pio_hw;
...
{
    TRICE( ID(13302),"PIO_LL: sync slow start signal\n");
    pio_hw_t *pio_hw = cd->pio_hw;
    uint32_t eom_idx = can2040_offset_sync_found_end_of_message;
...
{
    TRICE( ID(15606),"PIO_LL: setting Match SM (key: %08x)\n", match_key);
    pio_hw_t *pio_hw = cd->pio_hw;
...
{
    TRICE( ID(10023),"PIO_LL: Resetting TX SM\n");
    pio_hw_t *pio_hw = cd->pio_hw;
...
{
    TRICE( ID(14852),"PIO_LL: Queuing message for TX SM (count: %d)\n", count);
    pio_hw_t *pio_hw = cd->pio_hw;
    pio_tx_reset(cd);
...
{
    TRICE( ID(11152),"PIO_LL: Queuing ack for TX SM\n");
    pio_hw_t *pio_hw = cd->pio_hw;
    pio_tx_reset(cd);
...
{
    TRICE( ID(13733),"PIO_LL: Setting maytx and rx irq\n");
    pio_hw_t *pio_hw = cd->pio_hw;
...
{
    TRICE( ID(15628),"PIO_LL: Setting maytx, matched, and rx irq\n");
    pio_hw_t *pio_hw = cd->pio_hw;
...
{
    TRICE( ID(14129),"PIO_LL: Setting maytx, ackdone, and rx irq\n");
    pio_hw_t *pio_hw = cd->pio_hw;
...
{
    TRICE( ID(13279),"PIO_LL: Enabling additional maytx irq\n");
    pio_hw_t *pio_hw = cd->pio_hw;
...
{
    TRICE( ID(12531),"PIO_LL: Setting rx irq\n");
    pio_hw_t *pio_hw = cd->pio_hw;
...
{
    TRICE( ID(14873),"PIO_LL: setting up SM\n");
...
{
    TRICE( ID(13037),"PIO_LL: setting initial state\n");
...
tx_schedule_transmit(struct can2040 *cd)
{
    if (cd->tx_state == TS_QUEUED && !pio_tx_did_conflict(cd)) {
        TRICE( ID(14690),"TX_STT: Attempting to queue but already transmitting\n");
    ...
    if (cd->tx_push_pos == cd->tx_pull_pos) {
        TRICE( ID(15222),"TX_STT: Attempting to queue but no messages to send\n");
    ...
    TRICE( ID(12901),"TX_STT: Transmitting next message in queue\n");
...
tx_inject_ack(struct can2040 *cd, uint32_t match_key)
{
    if (cd->tx_state == TS_QUEUED && !pio_tx_did_conflict(cd)
        ...
        TRICE( ID(11635),"TX_STT: RX too slow, can't ack RX\n");
        return -1;
    }

    TRICE( ID(10653),"TX_STT: Acking received transmission\n");
...
tx_check_local_message(struct can2040 *cd)
{
    ...
    if (qt->crc == cd->parse_crc && tm->id == pm->id && tm->dlc == pm->dlc
        ...
        TRICE( ID(13866),"TX_STT: RX heard transmitted message\n");
        return 1;
    }

    TRICE( ID(12447),"TX_STT: RX found a different message from queued TX\n");
    return 0;
}
...
{
    TRICE( ID(10312),"RPT_CB: Report Note Message Start\n");
...
{
    TRICE( ID(10902),"RPT_CB: Report Note CRC Start\n");
...
{
    TRICE( ID(13575),"RPT_CB: Report Note CRC Success\n");
...
{
    TRICE( ID(12705),"RPT_CB: Report Note ACK Success\n");
...
{
    TRICE( ID(12150),"RPT_CB: Report Note EOF Found\n");
...
{
    TRICE( ID(11012),"RPT_CB: Report Note Parse Error\n");
    if (cd->report_state != RS_IDLE) {
        cd->report_state = RS_IDLE;
        pio_match_clear(cd);
    }
...
{
    TRICE( ID(12440),"RPT_CB: Report line ackdone [acked rx msg]\n");
...
{
    TRICE( ID(11481),"RPT_CB: Report line matched [fast tx]\n");
...
{
    TRICE( ID(10912),"RPT_CB: Report line maytx [slow tx]\n");
...
{
    TRICE( ID(12706), "RX_STT: Setting parse state to %d (need %d bits)\n", state, num_bits);
...
{
    TRICE( ID(12453),"RX_STT: Transitioning to MS_DISCARD\n");
...
{
    TRICE( ID(15428),"RX_STT: Line stuck active\n");
...
{
    TRICE( ID(10555),"RX_STT: Line state passive\n");
    ...
    TRICE( ID(12520),"RX_STT: Raw Stuffed bits: 0x%08x, count stuff: 0x%08x, stuffed_bits: 0x%08x\n", cd->unstuf.stuffed_bits, cd->unstuf.count_stuff, stuffed_bits);
    uint32_t dom_bits = ~stuffed_bits;
    if (!dom_bits) {
        TRICE( ID(12855),"RX_STT: Counter overflow in Sync state machine\n");
    ...
    TRICE( ID(13504),"RX_STT: Error switching to SOF 0x%08x\n", dom_bits);
...
{
    TRICE( ID(11485),"RX_STT: Transitioning to MS_CRC\n");
    cd->parse_crc &= 0x7fff;
...
{
    TRICE( ID(14343),"RX_STT: Transitioning to MS_DATA0 (0x%08x)\n", data);
    if (data & (0x03 << 4)) {
...
{
    TRICE( ID(12795),"RX_STT: Updating data state START (0x%08x)\n", data);
...
{
    TRICE( ID(13818),"RX_STT: Updating data state HEADER (0x%08x)\n", data);
...
{
    TRICE( ID(12516),"RX_STT: Updating data state EXT HEADER (0x%08x)\n", data);
...
{
    TRICE( ID(10154),"RX_STT: Updating data state DATA0 (0x%08x)\n", data);
...
{
    TRICE( ID(13644),"RX_STT: Updating data state DATA1 (0x%08x)\n", data);
...
{
    TRICE( ID(11425),"RX_STT: Updating data state CRC (0x%08x)\n", data);
...
{
    TRICE( ID(15265),"RX_STT: Updating data state ACK (0x%08x)\n", data);
    if (data != 0x01) {
        data_state_go_discard(cd);
        return;
    }
...
{
    TRICE( ID(12515),"RX_STT: Updating data state EOF0 (0x%08x)\n", data);
...
{
    TRICE( ID(11341),"RX_STT: Updating data state EOF1 (0x%08x)\n", data);
...
{
    TRICE( ID(10109),"RX_STT: Updating data state discard (0x%08x)\n", data);
    data_state_go_discard(cd);
}
...
{
    TRICE( ID(14145),"IRQ_CB: Processing RX data (0x%08x)\n", rx_data);
...
{
    TRICE( ID(11558),"IRQ_CB: >>> PIO IRQ Fired\n");
    pio_hw_t *pio_hw = cd->pio_hw;
    ...
    while (likely(ints & PIO_IRQ0_INTE_SM1_RXNEMPTY_BITS)) {
        ...
        ints = pio_hw->ints0;
        if (likely(!ints)) {
            TRICE( ID(11322), "IRQ_CB: <<< PIO IRQ Done\n");
            return;
        }
    }
