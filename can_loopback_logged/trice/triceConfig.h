...
#define TRICE_READ_TICK16 (time_us_32() & 0xFFFF)
#define TRICE_READ_TICK32 time_us_32()
...
#define TRICE_UART 1 //!< Enable and set UART for serial output.
...
#endif
#define TRICE_HALF_BUFFER_SIZE 5000 //!< This is the size of each of both buffers. Must be able to hold the max TRICE burst count within TRICE_TRANSFER_INTERVAL_MS or even more, if the write out speed is small. Must not exceed SEGGER BUFFER_SIZE_UP
...
#define TRICE_ENTER_CRITICAL_SECTION { uint32_t save = save_and_disable_interrupts(); {

//! TRICE_LEAVE_CRITICAL_SECTION restores interrupt state.
#define TRICE_LEAVE_CRITICAL_SECTION } restore_interrupts(save); }
...
TRICE_INLINE void triceSendBuffer(unsigned char const * buf, size_t len) {
    while (len--) {
        putchar_raw(*buf++);
    }
}
